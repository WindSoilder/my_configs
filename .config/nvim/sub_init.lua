...
do
  ...
  vim.keymap.set('n', '<leader>M', function()
    ...
    local module = vim.fn.fnamemodify(filename, ':.'):gsub('\\', '/'):gsub('%.py$', ''):gsub('/__init__$', ''):gsub('/', '.')
    vim.fn.setreg('+', module)
    vim.notify('Copied Python module: ' .. module)
  end, { desc = 'Copy current file as Python module' })
...
do
  ...
  -- Load the colorscheme here. Personal config keeps kanagawa as the active theme,
  -- with tokyonight and catppuccin installed for quick switching.
...
do
  ...
  --
  -- snacks.nvim is a collection of QoL improvements. Its picker module is a
  -- fuzzy finder for Neovim help, files, buffers, diagnostics, LSP symbols,
  ...
  vim.keymap.set('n', '<leader>sn', function() picker.files { cwd = vim.fn.stdpath 'config', follow = true } end, { desc = '[S]earch [N]eovim files' })
end
...
do
  ...
  vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(event)
      ...
      if client and client:supports_method('textDocument/inlayHint', event.buf) then
        ...
        -- Wind setup:
        -----------------
        map('<leader>ih', toggle_inlay_hints, 'Toggle [I]nlay [H]ints')
      end
    end,
  })
  ...
  -- Wind setup:
  -----------------
  -- LSP servers and clients communicate supported features during startup.
  -- nvim-ufo uses folding range capability.
  ...
  local servers = {
    ...
    -- Wind setup:
    -----------------
    -- For python lsp, third-party plugins can be installed inside pylsp with `:PylspInstall`.
    -- Example:
    -- pylsp = {
    --   settings = {
    --     pylsp = {
  ...
  -- Wind setup:
  -----------------
  -- Nushell ships its LSP through `nu --lsp`, outside mason-lspconfig.
  vim.api.nvim_create_autocmd('FileType', {
    pattern = 'nu',
    callback = function(args)
      local filename = vim.api.nvim_buf_get_name(args.buf)
      local root_dir = vim.fs.root(filename, { '.git' }) or vim.fn.fnamemodify(filename, ':p:h')
      vim.lsp.start {
        name = 'nushell',
        cmd = { 'nu', '--lsp' },
        root_dir = root_dir,
...
do
  ...
  local parsers = { 'bash', 'c', 'diff', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'nu', 'python', 'query', 'rust', 'vim', 'vimdoc' }
...
-- ============================================================
-- SECTION 10: PERSONAL EDITING / NAVIGATION PLUGINS
-- autopairs, aerial, ufo folds, copilot
-- ============================================================
do
  -----------------
  -- Wind setup:
  ...
  if vim.g.have_nerd_font then table.insert(personal_plugins, gh 'nvim-tree/nvim-web-devicons') end
  vim.pack.add(personal_plugins)

  -- Configure folds for nvim-ufo.
  vim.o.foldcolumn = '1'
  vim.o.foldlevel = 99
  ...
  require('nvim-autopairs').setup {
    ...
    ignored_next_char = '[%w%.]', -- Ignore alphanumeric and `.` next chars.
  }
